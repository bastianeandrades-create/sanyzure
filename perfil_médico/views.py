...
# Usamos Helvetica como fuente estándar de PDF, es la más clara y universal
...
def draw_wrapped_paragraph(c, text, x, y, max_width, style, indent=0):
...
def exportar_perfil_pdf(request):
    ...
    tipo_export = request.GET.get('tipo', 'todo')
    buffer = io.BytesIO()
    ...
    if tipo_export == 'todo':
        ...
        if perfil:
            ...
            y = draw_wrapped_paragraph(c, perfil.condiciones_medicas, margin, y, max_width, style_body, indent=0.2*inch)

            y = check_page_break(c, y, margin)
            c.setFont('Helvetica-Bold', 14)
            c.setFillColor(COLOR_SECUNDARIO)
            c.drawString(margin, y, "Alergias:")
            y = draw_wrapped_paragraph(c, perfil.alergias, margin, y, max_width, style_body, indent=0.2*inch)

            y = check_page_break(c, y, margin)
            c.setFont('Helvetica-Bold', 14)
            c.setFillColor(COLOR_SECUNDARIO)
            c.drawString(margin, y, "Medicamentos Fijos:")
            y = draw_wrapped_paragraph(c, perfil.medicamentos_fijos, margin, y, max_width, style_body, indent=0.2*inch)
            
            y = check_page_break(c, y, margin)
            c.setFont('Helvetica-Bold', 14)
            c.setFillColor(COLOR_SECUNDARIO)
            c.drawString(margin, y, "Contactos de Emergencia:")
            y = draw_wrapped_paragraph(c, perfil.contactos_emergencia, margin, y, max_width, style_body, indent=0.2*inch)
            y -= 40
    ...
    if not eventos_pasados.exists():
        ...
        for evento in eventos_pasados:
            ...
            y = draw_wrapped_paragraph(c, f"{fecha_hora} - {evento.titulo}", margin, y, max_width, style_body_bold, indent=indent_evento)
            
            # 2. Tipo de Evento
            c.setFont('Helvetica', 10)
            c.setFillColor(COLOR_TEXTO)
            y = draw_wrapped_paragraph(c, f"<b>Tipo:</b> {evento.get_tipo_evento_display()}", margin, y, max_width, style_body, indent=indent_evento)
            
            # 3. Descripción (si existe)
            if evento.descripcion:
                y = draw_wrapped_paragraph(c, f"<b>Descripción:</b> {evento.descripcion}", margin, y, max_width, style_body, indent=indent_evento)
            ...
            if evento.tipo_evento != 'cita':
                ...
                y = draw_wrapped_paragraph(c, estado, margin, y, max_width, style_body, indent=indent_evento)
